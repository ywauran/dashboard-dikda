      ...
      <div className="flex flex-col px-9">
        ...
        <div className="p-4 overflow-x-auto shadow-md">
          <table className="table">
            ...
            <tbody>
              {data.map((item, index) => {
                // Split the "Org Unit ID"
                const orgUnitIdParts = item["Org Unit ID"].split("_");
                // Extract the desired part (in this case, the first part)
                const extractedPart = orgUnitIdParts[0];

                return (
                  <tr key={index}>
                    <td>{startNumber + index}</td>
                    <td>{extractedPart}</td> {/* Display the extracted part */}
                    <td>{item["Org Unit Name"]}</td>
                    <td>
                      <Link
                        to={`/detail/smk/${extractedPart}`}
